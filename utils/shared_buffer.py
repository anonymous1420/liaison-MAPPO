...
class SharedReplayBuffer(object):
    ...
    def __init__(self, args, num_agents, obs_space, cent_obs_space, act_space):
        ...
        self.flag = np.ones((self.episode_length+1, self.n_rollout_threads, self.num_red,1), dtype=np.uint8)
        self.share_obs = np.zeros((self.episode_length + 1, self.n_rollout_threads, self.num_red, *share_obs_shape),
                                  dtype=np.float32)
        self.obs = np.zeros((self.episode_length + 1, self.n_rollout_threads, self.num_red, *obs_shape), dtype=np.float32)

        self.rnn_states = np.zeros(
            (self.episode_length + 1, self.n_rollout_threads, self.num_red, self.recurrent_N, self.hidden_size),
            dtype=np.float32)
        self.rnn_states_critic = np.zeros_like(self.rnn_states)

        self.value_preds = np.zeros(
            (self.episode_length + 1, self.n_rollout_threads, self.num_red, 1), dtype=np.float32)
        self.returns = np.zeros_like(self.value_preds)
        self.advantages = np.zeros(
            (self.episode_length, self.n_rollout_threads, self.num_red, 1), dtype=np.float32)

        if act_space.__class__.__name__ == 'Discrete':
            self.available_actions = np.ones((self.episode_length + 1, self.n_rollout_threads, self.num_red, act_space.n),
                                             dtype=np.float32)
        else:
        ...
        self.actions = np.zeros(
            (self.episode_length, self.n_rollout_threads, self.num_red, act_shape), dtype=np.float32)
        self.action_log_probs = np.zeros(
            (self.episode_length, self.n_rollout_threads, self.num_red, act_shape), dtype=np.float32)
        self.rewards = np.zeros(
            (self.episode_length, self.n_rollout_threads, self.num_red, 1), dtype=np.float32)

        self.masks = np.ones((self.episode_length + 1, self.n_rollout_threads, self.num_red, 1), dtype=np.float32)
    ...
    def insert(self, share_obs, obs, rnn_states_actor, rnn_states_critic, actions, action_log_probs,
               value_preds, rewards, masks, flag, bad_masks=None, active_masks=None, available_actions=None):
        """
        ...
        self.flag[self.step + 1] = flag.copy()
    ...
    def chooseinsert(self, share_obs, obs, rnn_states, rnn_states_critic, actions, action_log_probs,
        ...
        self.rewards[self.step] = rewards.copy()
        self.masks[self.step + 1] = masks.copy()
        self.flag[self.flag + 1] = flag.copy()
        if bad_masks is not None:
            self.bad_masks[self.step + 1] = bad_masks.copy()
        if active_masks is not None:
    ...
    def recurrent_generator(self, advantages, num_mini_batch, data_chunk_length):
        ...
        for indices in sampler:
            ...
            for index in indices:
                ...
                flag_batch.append(flag[ind:ind + data_chunk_length])
            ...
            flag_batch = np.stack(flag_batch, axis=1)
            ...
            flag_batch = _flatten(L, N, flag_batch)

            yield share_obs_batch, obs_batch, rnn_states_batch, rnn_states_critic_batch, actions_batch,\
                  value_preds_batch, return_batch, masks_batch, active_masks_batch, old_action_log_probs_batch,\
                  adv_targ, flag_batch, available_actions_batch
